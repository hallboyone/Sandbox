...
err = struct("avg", [], "stdDev", [], "max", []);
%Error values
centerPoints = repmat([Par(1), Par(2)], n, 1);
diffVects = centerPoints - XY;
diffNorms = vecnorm(diffVects')';

err.avg = sum(diffNorms - Par(3))/n;
err.stdDev = sqrt(sum((diffNorms - Par(3)).^2)/n);
err.max = max(abs(diffNorms - Par(3)));
